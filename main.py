...
def format_transaction_report(result, header=None):
    ...
    report_lines += [
        ...
        f"{'+/-':<4} {'Nonce':<7} {'Val':<6} {'Amount':<14} {'Status':<25} {'Sig':<7} {'Function' :<20}",
    ...
    for tx in result['pending_transactions']:
        ...
        report_lines.append(
            f"{status_prefix:<4} {tx['nonce']:<7} {tx['validator_id']:<6} {tx['amount']:<14,.1f} {tx['status']:<25} {tx.get('signature_count', 0)}/{tx.get('confirmations_required', 0):<7} {tx.get('func','N/A'):<20}"
        )
